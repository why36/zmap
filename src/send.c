...
int send_run(sock_t st, shard_t *s)
{
	...
	const int round_lowRate = 10;				// represent the round count before switching to high rate 10
	...
	while (1) {
		...
		if (current_ip == ZMAP_SHARD_DONE) {
			if (current_round == zconf.packet_streams) {
				log_debug(
					"send",
			...
			if (send_rate < slow_rate) {
				...
				if (!interval || (count % interval == 0)) {
					...
					double multiplier =
					    ...
					    (t - last_time) /
					    (zconf_rate_local / zconf.senders);
		...
		count++;
		uint32_t src_ip = get_src_ip(current_ip, current_round);
		uint32_t validation[VALIDATE_BYTES / sizeof(uint32_t)];
		validate_gen(src_ip, current_ip, (uint8_t *)validation);
		uint8_t ttl = zconf.probe_ttl;
		size_t length = 0;
		int path_info = ((current_round - 1) / (round_lowRate + round_highRate)) / 3;		// TOCHANGE
		// int path_info = 0;
		...
		zconf.probe_module->make_packet(
			...
			((path_info << 4) | (round_info & 0xF)), probe_data);
