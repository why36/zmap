...
sudo ./zmap     -w $target -v 5 -M udp --probe-args=latency:0000 -p 65535  -r $rate -P $per_ip --batch=$batch -c $coolTime --sender-threads=$sender_threads      -l $log_file -u $status_file --output-file=$output_file --output-module=csv --output-fields="icmp_responder,saddr,icmp_type,icmp_code,icmp_round,icmp_path,icmp_timestamp,icmp_rtt" --metadata-file=$metadata_file > $error_file
