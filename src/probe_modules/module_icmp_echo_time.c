...
static int icmp_validate_packet_LBfixed (const struct ip *ip_hdr, uint32_t len,
				uint32_t *src_ip, uint32_t *validation)
{
	if (ip_hdr->ip_p != IPPROTO_ICMP) {
		return 0;
...
static void icmp_echo_process_packet_LBfixed(const u_char *packet,
				     UNUSED uint32_t len,
				     fieldset_t *fs,
				     UNUSED uint32_t *validation,
				     UNUSED struct timespec ts)
...
probe_module_t module_icmp_echo_time = {
    ...
    .print_packet = &icmp_echo_print_packet,
    .process_packet = &icmp_echo_process_packet_LBfixed,
    .validate_packet = &icmp_validate_packet_LBfixed,
    .close = NULL,
    .output_type = OUTPUT_TYPE_STATIC,
    .fields = fields_LBfixed,
    .numfields = 10
